...
namespace Client.App
{
    internal class Program
    {
        ...
        public static int Main(string[] args)
        {
            ...
            const string clientIdEnvVar = "DDOAAMC_ClientId";
            ...
            app.Command("scenario1", configCmd =>
            {
                ...
                var clientId = configCmd
                    .Option("-ci|--clientId", $"Client ID (or env:'{clientIdEnvVar}')",
                        CommandOptionType.SingleValue)
                    .IsRequired();
                ...
                {
                    if (!clientId.Values.Any())
                    {
                        var value = Environment.GetEnvironmentVariable(clientIdEnvVar);
                        if (!string.IsNullOrWhiteSpace(value)) clientId.Values.Add(value);
                    }
            ...
            app.Command("scenario3", configCmd =>
            {
                var name = configCmd
                    .Option("-n|--name", "Mesh Name",
                        CommandOptionType.SingleValue)
                ...
                configCmd.OnExecute(() =>
                {
                    ...
                    containerBuilder.RegisterType<Scenarios.Third.InputManager>()
                        .AsSelf();

                    var arguments = new Scenarios.Third.Arguments
                    {
                        Name = name.Value(),
                        ImageName = imageName.Value(),
                        ClientId = clientId.Value(),
                    ...
                    var inputManager = container.Resolve<Scenarios.Third.InputManager>();
